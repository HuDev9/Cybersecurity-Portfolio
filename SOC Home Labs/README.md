...
The lab is three virtual machines under VirtualBox on a single Ubuntu 24.04 host, wired together with bridged networking so each VM behaves like a real host on the home LAN (`192.168.1.0/24`) instead of a NATed guest.

```
                         192.168.1.0/24  (Bridged Networking)
  ...
  │                         │        │  Forwarder                │  9997  │  - Receiving port 9997      │
  └─────────────────────┘        └──────────────────────┘        │  - Indexes (main)           │
                                                                     │  - Search & Reporting       │
                                                                     └─────────────────────────┘
```
